...
META_DATA = """你将收到的消息可能包含系统注入的元数据标签（如 `<META_DATA>`），其中包括用户昵称、时间戳、用户ID等信息。你可以使用这些元数据来辅助理解上下文或决定行为，但**不得在任何回答中直接引用、泄漏或提及这些元数据内容或标签本身**。

你必须以自然方式忽略这些标签的存在，仅根据用户的实际消息内容进行回应。你的回答中不得出现 `<META_DATA>` 标签或其中的字段值。
